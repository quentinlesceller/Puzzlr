...
class AsymmetricCryptoManager: NSObject {
    ...
    func getPublicKeyData(tagData : String) -> NSData? {
        let parameters = [
            kSecClass as String: kSecClassKey,
            kSecAttrKeyType as String: kSecAttrKeyTypeRSA,
    ...
    func getPublicKeyReference(tagData : String) -> SecKeyRef? {
        let parameters = [
            kSecClass as String: kSecClassKey,
            kSecAttrKeyType as String: kSecAttrKeyTypeRSA,
            kSecAttrApplicationTag as String: tagData,
    ...
    func getPrivateKeyReference() -> SecKeyRef? {
        let parameters = [
            kSecClass as String: kSecClassKey,
    ...
    func keyPairExists(tagData : String) -> Bool {
        return self.getPublicKeyData(tagData) != nil
    }
    ...
    func deleteSecureKeyPair(tagData : String,completion: ((success: Bool) -> Void)?) {
        // private query dictionary
        let deleteQuery = [
            kSecClass as String: kSecClassKey,
            kSecAttrApplicationTag as String: tagData,
    ...
    func addPublicKeytoKeyPair(publicKeyTag : String, publicKey : NSData, completion : ((success: Bool, errorMessage : String) -> Void)?) {
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0)) { () -> Void in
            
            let publicKeyData = publicKey
            
            let tag = publicKeyTag
            let tagData = NSData(bytes: tag, length: tag.lengthOfBytesUsingEncoding(NSUTF8StringEncoding))
            self.removeKeyWithTagData(tagData)
            ...
            keyDict.setObject(publicKeyData,              forKey: kSecValueData as! NSCopying)
            ...
            if secStatus != noErr && secStatus != errSecDuplicateItem {
                ...
                dispatch_async(dispatch_get_main_queue(), { completion?(success: false, errorMessage: "Provided key couldn't be added to the keychain")
                })
            }
            
            self.keyTags.append(tagData)
        ...
        dispatch_async(dispatch_get_main_queue(), { completion?(success: true, errorMessage: "success")
        })
    ...
    func encryptMessageWithPublicKey(message: String, publicKeyTagData : String,completion: (success: Bool, data: NSData?, error: AsymmetricCryptoException?) -> Void) {
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0)) { () -> Void in
            
            if let publicKeyRef = self.getPublicKeyReference(publicKeyTagData) {
                print("dedans")
    ...
    func verifySignaturePublicKey(data: NSData,publicKeyTagData :String, signatureData: NSData, completion: (success: Bool, error: AsymmetricCryptoException?) -> Void) {
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0)) { () -> Void in
            var error: AsymmetricCryptoException? = nil
